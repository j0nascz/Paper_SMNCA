...
are either placed in the same cluster in both partitions or in different clusters in both partitions.
However, since random clusterings can also exhibit non-zero agreement,
the ARI corrects this value by adjusting for chance.

Formally, the ARI is defined as:
\begin{equation}
    ARI = \frac{RI - E[RI]}{\max(RI) - E[RI]}
\end{equation}
where $RI$ is the Rand Index, $E[RI]$ is the expected value of the Rand Index for random clusterings, 
...
for this by subtracting the expected mutual information under a random model.

The AMI is defined as:
\begin{equation}
    AMI = \frac{MI - E[MI]}{\max(MI) - E[MI]}
\end{equation}
where $MI$ is the Mutual Information, $E[MI]$ is the expected value of the Mutual
...
across partitions. It is relatively easy to interpret. However, it can be sensitive
to imbalanced cluster sizes.
On the other hand, AMI is information-theoretic and evaluates how much information is shared 
between clusterings. It is generally more robust to imbalanced cluster distributions
and performs well when the number of clusters differs between the partitions.
The ARI is often preferred when interpretability and pairwise consistency
are important, particularly in the case of balanced cluster sizes.
...
Both indices are valuable tools for community comparison.
Later in the results section, we will apply ARI to compare different
